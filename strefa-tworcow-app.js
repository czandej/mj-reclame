...
(function () {
  ...
  async function requireUser(sb) {
    ...
    if (!user) {
      message("Zaloguj się, aby korzystać z panelu twórcy.", true);
      return null;
    }
    return user;
  }
  ...
  async function initHomeProductVisualizations(sb) {
    ...
    const items = (data || []).filter(v => v.submissions && v.submissions.status === "zaakceptowane" && v.submissions.type === "grafika");
  ...
  function ensurePublishModal() {
      ...
      <div class="publish-modal-backdrop" id="author-publish-modal" hidden>
        <div class="publish-modal" role="dialog" aria-modal="true" aria-labelledby="publish-modal-title">
          ...
          <form class="publish-modal-body" id="author-publish-form">
            ...
            <label class="check">
              <input type="checkbox" required>
              <span>Akceptuję <a href="regulamin-strefy-tworcow.html" target="_blank" rel="noopener">Regulamin Strefy Twórców</a> i zapoznałem się z <a href="polityka-prywatnosci.html" target="_blank" rel="noopener">Polityką prywatności</a>.</span>
            </label>

            <div class="publish-modal-actions">
              <button class="confirm-publish" type="submit">Publikuję tekst w czytniku</button>
        ...
        myList.addEventListener("click", async function (e) {
          ...
          form.addEventListener("submit", async function (event) {
            ...
            const { data: publishedRow, error } = await sb.from("submissions").update({
              ...
              publication_note: "Autor potwierdził publikację tekstu w czytniku MJ Reclame oraz zaakceptował wymagane oświadczenia."
            })
            .eq("id", id)
            ...
            if (!publishedRow) {
              return message("Nie udało się opublikować tekstu. Tekst może nie mieć statusu „zaakceptowane” albo konto nie ma uprawnień do tej pracy. Odśwież stronę i sprawdź status w „Moje prace”.", true);
            }
    ...
    if (adminList && sb) {
      (async function loadAdmin() {
        ...
        if (error) return message(error.message, true);
        if (!data || data.length === 0) {
          adminList.innerHTML = "<p>Brak zgłoszeń.</p>";
          return;
        }
        ...
        const rows = await Promise.all(data.map(async item => {
          ...
          return `
            <article class="submission-item" data-id="${item.id}">
              ...
              ${preview}
              <div class="submission-actions">
                ${item.status !== "opublikowane" && item.status !== "ukryte" ? `<button data-action="zaakceptowane" data-id="${item.id}">Akceptuj</button>` : ""}
                ${item.status !== "opublikowane" ? `<button class="reject" data-action="odrzucone" data-id="${item.id}">Odrzuć</button>` : ""}
                ${item.type === "tekst" && item.status === "opublikowane" ? `<button class="hide-reader" data-action="ukryte" data-id="${item.id}">Ukryj z czytnika</button>` : ""}
              </div>
            </article>
          `;
        }));
  ...
  function statusLabel(status) {
    if (status === "zaakceptowane") return "zaakceptowane — czeka na decyzję autora";
    if (status === "opublikowane") return "opublikowane w czytniku";
    if (status === "ukryte") return "ukryte";
    if (status === "odrzucone") return "odrzucone";
    return "oczekuje";
  }
